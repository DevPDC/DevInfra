    ...
    <div class="row">
        <div class="col-md-3">
            <div class="card card-info">
                <div class="card-header">
                    ...
                    <strong>Service Categories <span class="label label-primary">{{ $categories->count() }}</span></strong>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                        ...
                        <tbody>
                            @foreach($categories as $category)
                                <tr>
                                    <td>{{ $category->id }}</td>
                                    <td>
                                        <a href="{{ route('category.show', $category->id) }}" class="btn btn-danger btn-sm btn-block"><i class="fa fa-eye"></i>
                                            {{ $category->category_name }}
                                        </a>
                                    </td>
                                    <td>{{ $category->category_abbr }}</td>
                                    <td><span class="badge badge-warning badge-md">{{ $category->posts->count() }}</span></td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
        ...
        <div class="col-md-9">
            <div class="card card-primary">
                <div class="card-header">
                    <i class="fa fa-table"></i>
                    <strong>List of Unfiltered Service Requests</strong>
        ...
        <script>    
                $(document).ready(function() {
                    $('.dataTable').DataTable({
                        processing: true,
                        serverSide: true,
                        ajax: '{{ route('api.allUnfilteredPosts') }}'
                    });
                });
        </script>
    @endsection
